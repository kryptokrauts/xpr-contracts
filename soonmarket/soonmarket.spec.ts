...
import {
  ...
  initialAdminColEdit,
  transferNfts,
} from '../helpers/atomicassets.helper.ts';
import {
  Auction,
  addTokens,
  announceAuction,
...
const soonmarket = blockchain.createContract(
  'soonmarket',
  'soonmarket/target/soonmarket.contract',
);

// deploy contracts required for testing
const eosioToken = blockchain.createContract(
  'eosio.token',
  'node_modules/proton-tsc/external/eosio.token/eosio.token',
);
const atomicassets = blockchain.createContract(
  'atomicassets',
  'external/atomicassets/atomicassets',
);
const atomicmarket = blockchain.createContract(
  'atomicmarket',
  'external/atomicmarket/atomicmarket',
);
const xtokens = blockchain.createContract(
  'xtokens',
  'node_modules/proton-tsc/external/xtokens/xtokens',
);
const xmdToken = blockchain.createContract(
  'xmd.token',
  'node_modules/proton-tsc/external/xtokens/xtokens',
);
// in real world not using xtokens contract, but just simulating with xtokens for testing
const loanToken = blockchain.createContract(
  'loan.token',
  'node_modules/proton-tsc/external/xtokens/xtokens',
);

// create accounts
const [powerofsoon, protonpunk, pixelheroes, marco, mitch] =
...
beforeEach(async () => {
  ...
  // tokens
  await mintTokens(eosioToken, XPR.name, XPR.precision, 10000000.0, 4, [
    marco,
    mitch,
  ]);
  await mintTokens(xtokens, XBTC.name, XBTC.precision, 21000000.0, 5, [
    marco,
    mitch,
  ]);
  await mintTokens(xtokens, XETH.name, XETH.precision, 100000000.0, 20, [
    marco,
    mitch,
  ]);
  await mintTokens(
  ...
  await mintTokens(xtokens, XMT.name, XMT.precision, 66588888.0, 5000, [
    marco,
    mitch,
  ]);
  await mintTokens(xtokens, METAL.name, METAL.precision, 666666666.0, 20000, [
    marco,
    mitch,
  ]);
  await mintTokens(loanToken, LOAN.name, LOAN.precision, 100000000.0, 20000, [
    marco,
    mitch,
  ]); // how to define unlimited?
  await mintTokens(xmdToken, XMD.name, XMD.precision, 100000000.0, 20000, [
    marco,
    mitch,
  ]); // how to define unlimited?
  // atomicmarket
  ...
  await addTokens(atomicmarket, xtokens, [
    XBTC,
    XETH,
    XDOGE,
    XUSDC,
...
describe('SoonMarket', () => {
  describe('revert paths', () => {
    ...
    it('reject if auction is not started yet', async () => {
      ...
      const auction: Auction = atomicmarket.tables
        .auctions()
        .getTableRows(undefined, { limit: 1 })[0];
      await expectToThrow(
        transferNfts(
