...
class SoonMarket extends Contract {
  ...
  validateAuction(auctionId: string): u32 {
    ...
    const goldSpotAuctionDuration = <u32>(
      SafeMath.add(
        SafeMath.sub(auction.end_time, currentTimePoint().secSinceEpoch()),
        ONE_DAY,
      )
    );
  ...
  validateAndHandleSpot(
    ...
    if ('auction' == promoType) {
      ...
      check(
        1099511627776 == spotNftId || SILVER_SPOT_AUCTIONS_ENABLED,
        ERROR_INVALID_PROMOTION_TYPE_AUCTION_GOLD_ONLY,
      );
      ...
      const memoAction =
        spotType == 'gold' ? `auction ${goldSpotAuctionDuration}` : 'burn';
      ...
      const promotionEnd = <u32>(
        SafeMath.add(
          ...
          spotType == 'gold' ? TWO_WEEKS : ONE_WEEK,
        )
      );
      sendLogColPromo(
        this.contract,
