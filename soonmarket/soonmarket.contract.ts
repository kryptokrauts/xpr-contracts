...
class SoonMarket extends Contract {
    ...
    // other tables
    collectionsBlacklist: TableStore<CollectionsBlacklist> =
        new TableStore<CollectionsBlacklist>(this.receiver)
    collectionsVerified: TableStore<CollectionsVerified> =
        new TableStore<CollectionsVerified>(this.receiver)
    ...
    addToBlacklist(
        ...
        requireAuth(this.contract)
        check(
            this.collectionsBlacklist.get(collection.N) == null,
            ERROR_COLLECTION_ALREADY_BLACKLISTED,
        )
        ...
        const blacklistRow = new CollectionsBlacklist(
            collection,
            timestamp ? timestamp : currentTimePoint().secSinceEpoch(),
            comment,
            references,
        )
    ...
    delFromBlacklist(collection: Name): void {
        requireAuth(this.contract)
        const blacklistRow = this.collectionsBlacklist.requireGet(
            collection.N,
            ERROR_COLLECTION_NOT_BLACKLISTED,
        )
    ...
    addToVerified(
        ...
        requireAuth(this.contract)
        check(
            this.collectionsVerified.get(collection.N) == null,
            ERROR_COLLECTION_ALREADY_VERIFIED,
        )
    ...
    delFromVerified(collection: Name): void {
        requireAuth(this.contract)
        const verifiedRow = this.collectionsVerified.requireGet(
            collection.N,
            ERROR_COLLECTION_NOT_VERIFIED,
        )
    ...
    onReceive(from: Name, to: Name, asset_ids: u64[], memo: string): void {
        ...
        if (to == this.contract) {
            ...
            check(
                this.isValidPromotionType(memoWords[0]),
                ERROR_INVALID_PROMOTION_TYPE,
            )
            this.validateAndHandleSpot(
                asset_ids[0],
                memoWords[0],
                memoWords[1],
                from,
    ...
    checkIfVerified(collection: Name): void {
        ...
        check(
            verifiedRow != null || shielded,
            ERROR_COLLECTION_NEITHER_VERIFIED_NOR_SHIELDED,
        )
    }

    validateCollection(collection: Name): void {
        new TableStore<Collections>(ATOMICASSETS_CONTRACT).requireGet(
            collection.N,
            ERROR_COLLECTION_NOT_EXISTS,
        )
    ...
    validateAuction(auctionId: string): u32 {
        const auction = new TableStore<Auctions>(
            ATOMICMARKET_CONTRACT,
        ).requireGet(<u64>parseInt(auctionId), ERROR_AUCTION_NOT_EXISTS)
        const collection = new TableStore<Collections>(
            ATOMICASSETS_CONTRACT,
        ).requireGet(auction.collection_name.N, ERROR_COLLECTION_NOT_EXISTS)
        ...
        check(
            auction.end_time >
                SafeMath.add(currentTimePoint().secSinceEpoch(), ONE_HOUR),
            ERROR_AUCTION_EXPIRED_OR_CLOSE_TO_EXPIRATION,
        )
    ...
    validateAndHandleSpot(
        ...
        if ('auction' == promoType) {
            ...
            check(
                currentGlobals.goldSpotId == spotNftId ||
                    SILVER_SPOT_AUCTIONS_ENABLED,
                ERROR_INVALID_PROMOTION_TYPE_AUCTION_GOLD_ONLY,
            )
            ...
            if (currentGlobals.goldSpotId == spotNftId) {
                ...
                const asset = new TableStore<Assets>(
                    ATOMICASSETS_CONTRACT,
                    currentReceiver(),
                ).requireGet(spotNftId, 'fatal error - should never happen')
                check(
                    currentGlobals.silverSpotTemplateId == asset.template_id,
                    ERROR_INVALID_NFT_SILVER_SPOT_EXPECTED,
                )
                spotType = 'silver'
            }
            ...
            const memoAction =
                spotType == 'gold'
                    ? `auction ${goldSpotAuctionDuration}`
                    : 'burn'
            sendTransferNfts(
                currentReceiver(),
                POWEROFSOON,
            ...
            const promotionEnd = <u32>(
                SafeMath.add(
                    currentTimePoint().secSinceEpoch(),
                    spotType == 'gold'
                        ? currentGlobals.goldPromoDuration
                        : currentGlobals.silverPromoDuration,
                )
            )
            sendLogColPromo(
                this.contract,
